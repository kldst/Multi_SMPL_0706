...
def rot6d_to_rotmat(x: torch.Tensor) -> torch.Tensor:
    """
    將 6D rotation 表示轉成 3x3 rotation matrix。
    輸入 shape: (..., 6)
    輸出 shape: (..., 3, 3)
    """
    assert x.shape[-1] == 6, f"Expected last dim = 6, got {x.shape}"
    x = x.view(*x.shape[:-1], 3, 2)          # (..., 3, 2)
    a1 = x[..., :, 0]                        # (..., 3)
    a2 = x[..., :, 1]                        # (..., 3)

    b1 = F.normalize(a1, dim=-1)
    # 從 a2 中扣掉在 b1 方向上的投影，再正規化得到第二軸
    proj = (a2 * b1).sum(dim=-1, keepdim=True)
