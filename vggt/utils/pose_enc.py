...
def pose_encoding_to_extri_intri(
    pose_encoding, image_size_hw=None, pose_encoding_type="absT_quaR_FoV", build_intrinsics=True, eps: float = 1e-6  # e.g., (256, 512)
):
    ...
    if pose_encoding_type == "absT_quaR_FoV":
        ...
        # print(f"T: {T}, R: {R}")
        # print(f"fov_h: {fov_h}, fov_w: {fov_w}")

        if build_intrinsics:
            H, W = image_size_hw
            fy = (H / 2.0) / torch.tan(fov_h / 2.0).clamp(min=eps)
            fx = (W / 2.0) / torch.tan(fov_w / 2.0).clamp(min=eps)
            # print(f"fy: {fy}, fx: {fx}")
