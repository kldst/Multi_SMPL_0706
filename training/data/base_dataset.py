...
class BaseDataset(Dataset):
    ...
    def process_one_image(
        ...
        if track is not None:
            track = np.copy(track)
        # Auxiliary maps (e.g. an instance mask) transformed in-place with the
        ...
        # print(f"orginal image size: {image.shape}")
        # print(f"orginal intri_opencv: {intri_opencv}")
        ...
        # print(f"crop1 image size: {image.shape}")
        # print(f"crop1 intri_opencv: {intri_opencv}")
        ...
        if track is not None:
            ...
            track_np = np.asarray(track)
            x = track_np[..., 0]
            y = track_np[..., 1]

            inside = (x >= 0) & (x < W) & (y >= 0) & (y < H)
            confidences = inside.astype(np.float32)  # 1 inside, 0 outside
