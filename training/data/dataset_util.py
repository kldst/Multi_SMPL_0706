...
def crop_image_depth_and_intrinsic_by_pp(
    ...
    Returns:
        ...
        Note:
            Any arrays passed via ``extra_maps`` (e.g., auxiliary masks) are modified in-place
            following the exact same spatial operations applied to ``depth_map``.
    """
    ...
    image = _crop_array(image)
    if depth_map is not None:
        depth_map = _crop_array(depth_map)
    for key, value in extra_maps.items():
        extra_maps[key] = _crop_array(value)
        ...
        if (image.shape[:2] != target_shape).any():
            ...
            def _pad_array(arr):
                if arr is None:
                    return None
                if arr.ndim >= 3:
                    pad_cfg = ((0, pad_h), (0, pad_w)) + tuple((0, 0) for _ in range(arr.ndim - 2))
                else:
                    pad_cfg = ((0, pad_h), (0, pad_w))
                return np.pad(arr, pad_cfg, mode="constant", constant_values=0)

            image = _pad_array(image)
            if depth_map is not None:
                depth_map = _pad_array(depth_map)
            for key, value in extra_maps.items():
                extra_maps[key] = _pad_array(value)
...
def resize_image_depth_and_intrinsic(
    ...
    def _resize_map(arr):
        ...
        if arr.ndim == 2:
            ...
            raise ValueError("Unsupported array dimensionality for resizing")

    if depth_map is not None:
        depth_map = _resize_map(depth_map)
    for key, value in extra_maps.items():
        extra_maps[key] = _resize_map(value)
    ...
    for value in extra_maps.values():
        if value is not None:
            assert image.shape[:2] == value.shape[:2]
    return image, depth_map, intrinsic, track
...
def rotate_90_degrees(
    ...
    rotated_image, rotated_depth_map = rotate_image_and_depth_rot90(
        image, depth_map, clockwise, extra_maps=extra_maps
    )
...
def rotate_image_and_depth_rot90(image, depth_map, clockwise, extra_maps=None):
    """
    ...
    def _rotate_generic(arr):
        ...
        rotated = np.flip(rotated, axis=flip_axis)
        return np.copy(rotated)

    rotated_image = _rotate_generic(image)
    if depth_map is not None:
        rotated_depth_map = _rotate_generic(depth_map)
    if extra_maps:
        for key, value in extra_maps.items():
            extra_maps[key] = _rotate_generic(value)
