            ...
            for split_name in split_name_list:
                ...
                for seq_name, seq_data in annotation.items():
                    ...
                    if seq_name in self.invalid_sequence:
                        continue
                    # Some setups (e.g. a downloaded subset) may not contain all sequences
                    # referenced by the annotation files. Skip sequences that are not present
                    # in CO3D_DIR to avoid runtime FileNotFoundError in __getitem__.
                    seq_dir = osp.join(self.CO3D_DIR, c, seq_name)
                    if not osp.isdir(seq_dir):
                        continue
                    if not osp.isdir(osp.join(seq_dir, "images")):
                        continue
                    if self.load_depth:
                        if not osp.isdir(osp.join(seq_dir, "depths")):
                            continue
                        if not osp.isdir(osp.join(seq_dir, "depth_masks")):
                            continue
