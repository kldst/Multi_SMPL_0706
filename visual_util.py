...
def predictions_to_glb(
    ...
    Args:
        ...
        smpl_mesh: Optional SMPL mesh to include in the exported scene.
    ...
    pred_world_points = None
    pred_world_points_conf = None
    if "Pointmap" in prediction_mode:
        ...
        if "world_points" in predictions:
            ...
            print("Warning: no point data found; building camera-only scene")
    else:
        ...
        if "world_points_from_depth" in predictions:
            ...
            print("Warning: world_points_from_depth missing; building camera-only scene")
    ...
    if pred_world_points is None:
        ...
        if show_cam:
            for i in range(num_cameras):
                ...
                rgba_color = colormap(i / max(num_cameras, 1))
                current_color = tuple(int(255 * x) for x in rgba_color[:3])
                integrate_camera_into_scene(scene_3d, camera_to_world, current_color, scene_scale)
        if smpl_mesh is not None:
            scene_3d.add_geometry(smpl_mesh)
    ...
    if mask_green_bg:
        rgb_int = colors_rgb[:, :3]

        unique_colors, counts = np.unique(rgb_int, axis=0, return_counts=True)

        max_idx = counts.argmax()

        most_common_color = unique_colors[max_idx]   # 形狀 (3,) -> [R, G, B]
        most_common_count = counts[max_idx]

        print("Most common RGB color:", most_common_color, "count:", most_common_count)
        is_green = (
            (colors_rgb[:, 0] == 0) &   # R = 0
            (colors_rgb[:, 1] > 0) & # G = 255
            (colors_rgb[:, 2] == 0)     # B = 0
        )
        conf_mask = conf_mask & (~is_green)
...
def integrate_camera_into_scene(scene: trimesh.Scene, transform: np.ndarray, face_colors: tuple, scene_scale: float):
    ...
    if not hasattr(integrate_camera_into_scene, "_dbg_printed"):
        integrate_camera_into_scene._dbg_printed = True
        print(f"[DBG] integrate_camera_into_scene: scene_scale={scene_scale} cam_width={cam_width} cam_height={cam_height}")
